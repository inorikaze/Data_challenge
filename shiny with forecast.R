...
ui <- fluidPage(
  ...
  titlePanel(
    tags$h1(style = "font-size: 15px;", "For Renewable energy consumption and production, we applied neural network forecasting which make take some time to run.")
  ),
...
server <- function(input, output) {
  ...
  observeEvent(input$h_value2, {
    forecast_data2 <- reactive({
      forecast(dnn_fit_1,PI=T, h = input$h_value2)
    })
  ...
  observeEvent(input$h_value4, {
    forecast_data4 <- reactive({
      forecast(dnn_fit_2,PI=T, h = input$h_value4)
    })
